...
import { usePublicationDetail, usePublicationsPub } from "../hooks/api/usePublications";
import { useAuth } from "../contexts/authContext";
import { FaLock } from "react-icons/fa";
import { useTranslation } from "react-i18next";
import { useParams } from "react-router-dom";

const DetailPublications = () => {
  const { publicationId, publicationTitre } = useParams();
  const { isAuthenticated } = useAuth();
  const { data: publications = [], isLoading, error } = usePublicationsPub();
  const { data: detailPublication, isLoadingDetailPu, error: errorDetailCat } = usePublicationDetail(publicationId);
    ...
    if (isAuthenticated) {
      return (
         <a href={`/publications/detail/${publication.titre
              }/${publication.id}`} className="text-gray-700 leading-relaxed">
          ...
          <div className="relative mt-2">
            ...
            <div className="absolute inset-0 flex items-end justify-center pb-2">
              <div className="inline-flex items-center gap-2 bg-white/90 px-3 py-1 rounded-full border border-gray-200 shadow-sm">
                ...
                <a href="/login" className="text-xs font-medium text-gray-600 cursor-pointer hover:text-primary ">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        {isLoadingDetailPu ? (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-gray-500">{t('publications.loadingDetails')}</p>
          </div>
        ) : errorDetailCat ? (
          <div className="text-center py-16">
            <p className="text-red-600 text-lg font-semibold">{t('publications.errorDetails')}</p>
          </div>
        ) : detailPublication && (
          <section className="bg-white shadow-lg rounded-xl overflow-hidden mb-12">

            {/* Banner Image */}
            <div className="relative h-96">
              <img
                src={detailPublication.image_path || "/default-banner.jpg"}
                alt={detailPublication.titre}
            ...
            <div className="px-6 py-10 md:px-10">
              <div className="mb-6 flex flex-col md:flex-row items-center md:items-start justify-between gap-6">

                <div className="flex flex-col gap-2">
                  <p className="text-sm text-gray-500">
                    🖋️ <strong>{t("publications.author")}:</strong> {detailPublication.auteur}
                  </p>
                  <p className="text-sm text-gray-500">
                    📅 <strong>{t("publications.publishedOn")}:</strong>{" "}
                    {new Date(detailPublication.date_publication).toLocaleDateString("fr-FR", {
                      weekday: "long",
                      year: "numeric",
                      month: "long",
                      day: "numeric",
              ...
              {detailPublication.resume && (
                <div className="mb-8 text-left">
                  <h2 className="text-xl font-semibold text-gray-800 mb-2">{t('publications.summary')}</h2>
                  <p className="text-gray-700 leading-relaxed">{detailPublication.resume}</p>
                </div>
              )}

              {detailPublication.contenu && (
...
export default DetailPublications;
