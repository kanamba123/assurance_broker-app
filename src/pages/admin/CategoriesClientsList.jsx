...
import { useAddClientsCategoris, useClientsCategoris, useDeleteClientsCategories, useUpdateCategorie } from '../../hooks/api/useCientCategories';

const CategoriesClientsList = () => {
  const { data: categories = [], isLoading, error, refetch } = useClientsCategoris();
  const addCategorieMutation = useAddClientsCategoris();
  const updateCategorieMutation = useUpdateCategorie();
  const deleteCategorieMutation = useDeleteClientsCategories();
    ...
    <div className="min-h-screen bg-gradient-to-br p-1">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-secondary">Gestion des categorie pour client</h1>
          <button
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-primary">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">image_uri</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {categories.map((category) => (
                    <tr key={category.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                          {category.image_uri ? (
                            <><img src={category.image_uri ? category.image_uri :"-" } width="50px" /></>
                          ):(<>-</>)}
                      </td>
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
               ...
               <div>
                <label className="block font-medium mb-1">image_uri *</label>
                <input
                  type="text"
                  name="image_uri"
...
export default CategoriesClientsList;